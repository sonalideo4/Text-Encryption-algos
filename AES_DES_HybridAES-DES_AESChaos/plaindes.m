function [descipher] =plaindes(msgd,key)
%tic
 msg=msgd;
%MB=[];  
%olength=length(M);
...
K_PC1=K(PC1);  %parity drop key 64 to 56                
...
for i=1:16 
    ...
    R_Ki=mod(R_E+key(i,:),2); %xor(R_E,Ki(i,:))
...
t='';
length(msgbin_str)
for i=1:3:length(msgbin_str)
    %msgbin_str(i:i+23)
    %bin2dec(msgbin_str(i:i+23))
    t=strcat(t,msgbin_str(i));
    if (length(t)==8)
        num_str8=bin2dec(t);
        outp=horzcat(outp,native2unicode(num_str8));
        t='';
    end
end
descipher=outp
...
%final=[];
%for i=1:length(outp)
%final=[final,dec2bin(unicode2native(outp(i)),8)];
%final=horzcat(final,dec2bin(unicode2native(outp(i)),8));
%end
%msgbin;
%final
%length(final);
%final=str2num(final);
%isequal(final,msgbin)
%strcmp(num2str(msgbin),num2str(final))
%t=toc